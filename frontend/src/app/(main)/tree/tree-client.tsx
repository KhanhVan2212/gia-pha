      ...
      <div className="flex-1 flex gap-0 min-h-0">
        ...
        {editorMode && (
          <EditorPanel
            ...
            treeData={treeData}
            onAddChild={async (parentHandle, name, gender, birthYear) => {
              // Generate unique handles
              const childHandle = `p-${Date.now()}`;
              const person = selectedCard
                ? treeData?.people.find((p) => p.handle === selectedCard)
                : null;
              const gen = person ? (person as TreeNode).generation + 1 : 1;

              // 1. Create the new person in Supabase
              const { error: pErr } = await addPerson({
                handle: childHandle,
                displayName: name,
                gender,
                generation: gen,
              ...
              // 2. Find existing family for this parent
              let parentFamily = treeData?.families.find(
                (f) =>
                  f.fatherHandle === parentHandle ||
                  f.motherHandle === parentHandle,
              );

              // 3. If no family exists, create one automatically
              if (!parentFamily) {
                const familyHandle = `f-${parentHandle}-${Date.now()}`;
                const parentPerson = treeData?.people.find(
                  (p) => p.handle === parentHandle,
                );
                const isMale = parentPerson?.gender === 1;
                const newFamilyData = {
                  handle: familyHandle,
                  fatherHandle: isMale ? parentHandle : undefined,
                  motherHandle: isMale ? undefined : parentHandle,
                ...
                setTreeData((prev) =>
                  prev
                    ? {
                        ...
                        people: [
                          ...prev.people.map((p) =>
                            p.handle === parentHandle
                              ? {
                                  ...p,
                                  families: [...p.families, familyHandle],
                                }
                              : p,
                          ),
                          {
                        ...
                        families: [
                          ...prev.families,
                          { ...newFamilyData, children: [childHandle] },
                        ],
                      }
                    : null,
                );
              } else {
                // 4. Attach child to existing family
                const newChildren = [...parentFamily.children, childHandle];
                await supaUpdateFamilyChildren(
                  parentFamily.handle,
                  newChildren,
                );
                setTreeData((prev) =>
                    ...
                    ? {
                        ...
                        people: [
                          ...prev.people,
                          {
                            ...
                            isPatrilineal: gender === 1,
                            families: [],
                            parentFamilies: [parentFamily!.handle],
                          },
                        ],
                        families: prev.families.map((f) =>
                          f.handle === parentFamily!.handle
...
function EditorPanel({
  ...
  onDeletePerson: (handle: string) => Promise<void>;
  onReset: () => void;
  onClose: () => void;
}) {
      ...
      {!person ? (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {person && parentFamily && (
            <div className="p-3 border-b">
              <button
                className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded border border-blue-200 text-blue-600 hover:bg-blue-50 transition-colors"
              ...
              {isAddingChild && (
                <div className="mt-3 p-3 border rounded-lg bg-blue-50/50 space-y-3 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 space-y-1">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setNewChildGender(parseInt(e.target.value))
                        }
                      >
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      className="flex-1 bg-blue-600 text-white rounded py-1.5 text-xs font-medium hover:bg-blue-700 disabled:opacity-50"
          ...
          {/* Dangerous actions */}
          <div className="p-3 mt-auto border-t bg-red-50/30">
            <button
              className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded border border-red-200 text-red-600 hover:bg-red-100 transition-colors"
              onClick={() => onDeletePerson(person.handle)}
            >
              <Trash2 className="h-3.5 w-3.5" />
              Xóa thành viên vĩnh viễn
            </button>
          </div>
        </div>
      )}
